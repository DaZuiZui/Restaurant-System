...
public class MunuServerImpl implements MunuServer {
    ...
    public ArrayList<Menu> selectdata(Menu menu){
        return menuMapper.selectdata(menu);
    }
     ...
     */
    @Override
    public String queryByMenumenu(String menu){
        ArrayList<Menu> arrayList = menuMapper.queryByMenumenu(menu);
        if (arrayList.size() == 0){
            return null ;
        }
         String temp =  arrayList.get(0).getMenu();

         try {
             Integer integer = (Integer) redisTemplate.opsForHash().get("menu",temp);
    ...
    public String getALL() {
        return JSONArray.toJSONString(menuMapper.selectALL());
    }
}
